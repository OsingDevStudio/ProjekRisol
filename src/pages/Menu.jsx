...
const Menu = () => {
  ...
  return (
    <div className="min-h-screen bg-[#FDFDFD] relative overflow-x-hidden">
      ...
      <main className="relative z-10 pt-32 pb-44">
          ...
          <div className="flex justify-center gap-2 md:gap-3 mt-8 flex-wrap">
            {["semua", "makanan", "minuman"].map((cat) => (
              <button 
                ...
                className={`px-6 md:px-8 py-2.5 md:py-3 rounded-2xl font-black text-[9px] md:text-[10px] tracking-[0.15em] md:tracking-[0.2em] uppercase transition-all duration-300 ${
                  filter === cat 
        ...
        <div className="max-w-7xl mx-auto px-6 grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 gap-6 md:gap-8">
          {filteredData.map((item) => {
            const isAdded = cart[item.id] > 0 && cart[item.id] === tempQty[item.id];
            return (
              <div key={item.id} className="group bg-white rounded-[35px] border border-gray-100 shadow-sm hover:shadow-2xl transition-all duration-500 p-4 flex flex-col">
                <div className="relative h-48 md:h-52 rounded-[25px] overflow-hidden mb-5">
                ...
                <div className="text-center px-1 flex-1 flex flex-col">
                  <h3 className="text-md md:text-lg font-black text-gray-900 uppercase tracking-tight">{item.name}</h3>
                  <p className="text-gray-400 text-[10px] md:text-xs my-2 font-medium h-8 line-clamp-2 leading-relaxed">{item.desc}</p>
                  <h2 className="text-lg md:text-xl font-black text-red-500 mb-5">Rp {item.price.toLocaleString("id-ID")}</h2>
                  
                  <div className="mt-auto">
                    <div className="flex items-center justify-center gap-4 mb-4 bg-gray-50/50 p-1.5 rounded-[18px] border border-gray-100/50">
                      <button onClick={() => handleTempQty(item.id, -1)} className="w-9 h-9 bg-white rounded-xl shadow-sm flex items-center justify-center font-bold hover:bg-red-500 hover:text-white transition-all text-sm">-</button>
                      <span className="font-black text-md text-gray-800 w-6">{tempQty[item.id] || 0}</span>
                      <button onClick={() => handleTempQty(item.id, 1)} className="w-9 h-9 bg-white rounded-xl shadow-sm flex items-center justify-center font-bold hover:bg-green-500 hover:text-white transition-all text-sm">+</button>
                    </div>

                    <button 
                      onClick={() => addToCart(item)}
                      disabled={!tempQty[item.id] && !cart[item.id]}
                      className={`w-full py-4 rounded-[20px] font-black text-[9px] tracking-[0.15em] transition-all duration-300 ${
                        isAdded 
                    ...
                    >
                      {isAdded ? "TERSURVEI" : "TAMBAH PESANAN"}
                    </button>
                  </div>
                </div>
              </div>
      ...
      {/* Floating Checkout Bar - SLIM & RESPONSIVE */}
      {totalItems > 0 && (
        <div className="fixed bottom-6 md:bottom-10 left-0 right-0 z-[110] flex justify-center px-4">
          <div className="bg-gray-900/95 backdrop-blur-2xl text-white p-2 md:p-3 pl-6 md:pl-10 rounded-[28px] md:rounded-[40px] shadow-2xl flex items-center gap-5 md:gap-12 border border-white/10 w-full max-w-[420px] md:w-auto">
            
            <div className="flex flex-col min-w-[80px] md:min-w-[120px]">
              <span className="text-[8px] md:text-[10px] text-gray-400 font-black uppercase tracking-widest mb-0.5">
                {totalItems} Item
              </span>
            ...
            <button 
              ...
              className="bg-red-500 hover:bg-red-600 text-white flex-1 md:flex-none px-6 md:px-12 py-4 md:py-5 rounded-[22px] md:rounded-[32px] font-black text-[10px] md:text-xs tracking-[0.1em] md:tracking-[0.2em] transition-all active:scale-95 shadow-xl shadow-red-500/20 whitespace-nowrap"
            >
              ORDER SEKARANG
            </button>
          </div>
